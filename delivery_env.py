...
class DeliveryEnv(gym.Env):
    ...
    def _get_driver_pool(self, order):
        ...
        for driver in self.simulator.area_drivers.get(order.pickup_area, []):
            if driver.driver_id not in [
                ...
                if order.datetime.hour in self.simulator.driver_schedule.get(
    ...
    def _is_driver_working(self, driver, datetime):
        ...
        date = datetime.date()
        hour = datetime.hour
        return hour in self.simulator.driver_schedule.get(
            (driver.driver_id, date), set()
        )
    ...
    def render(self, mode="human"):
        ...
        print(
            f"Processing Order {self.current_order_index} → Hour: {self.simulator.orders[self.current_order_index].datetime.hour}, Weather Code: {self.simulator.weather.get_weather_code(self.simulator.orders[self.current_order_index].datetime)}"
        )
